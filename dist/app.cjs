...
// src/structs/item.struct.ts
var ItemStruct = z.object({
  id: z.string(),
  name: z.string().min(1, "O nome do item deve ter pelo menos 1 caractere.").max(50, "o nome do item deve ter no m\xE1ximo 50 caracteres."),
  description: z.string().optional(),
  value: z.number().nonnegative("valor do item n\xE3o pode ser negativo")
});
...
function configureExpressServer() {
  app.use(import_express.default.json()).post("/api/v1/createOrder", order_controller_default.getInstance().createOrderHandler()).post("/api/v1/createItem", item_controller_default.getInstance().createItemHandler()).get("/api/v1/getItems", item_controller_default.getInstance().getAllItemsHandler());
